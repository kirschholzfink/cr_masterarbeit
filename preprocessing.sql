...
/*
 ...
 This is equivalent to the following query:
    ...
    WHERE hist_message LIKE '%Build Started%'
       ...
       or hist_message like '%Change has been successfully cherry-picked as%'
       ...
       or hist_message like '%Change has been successfully merged into the git repository%';
 */
delete
from t_history
...
/*
 Delete all reviews in t_change that have not been commented on by both the author and a reviewer.
 */
delete
from t_change
where id not in
      (select *
       from (select comm.hist_changeId
             ...
             where ch.ch_authorAccountId != comm.hist_authorAccountId) as commentsByReviewers)
   or id not in
      (select *
       from (select comm.hist_changeid
             from t_change ch
                      join t_history comm on ch.id = comm.hist_changeId
             where ch.ch_authorAccountId = comm.hist_authorAccountId) as commentsByAuthor);

/*
 Delete comments in t_history that are associated with changes that were deleted.
 (This is required since there is no 'on delete/update cascade' action defined in db schema.)
 */
delete
from t_history
where hist_changeId not in (select id from t_change);
/*
 Delete all revisions in t_revision that are associated with changes that were deleted.
 (This is required since there is no 'on delete/update cascade' action defined in db schema.)
 */
delete
from t_revision
...
/*
 Delete all files in t_file that are associated with revisions that were deleted.
 (This is required since there is no 'on delete/update cascade' action defined in db schema.)
 */
delete
from t_file
